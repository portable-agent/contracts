...
- необязательный `context.actorId` приходит из сохранённого Action, а не из ответа модели;
...
`context` отделён от аргументов tool. В версии 2.6 он необязателен для совместимости с текущим
fake-календарём. Коннектор настоящего пользовательского аккаунта обязан отклонить вызов без
`actorId`. Gateway принимает такой контекст только от разрешённого service account.
